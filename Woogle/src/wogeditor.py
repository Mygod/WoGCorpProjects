...
#                    print "removing",text_element.get('id'),text_element,"from",text_element.parent
    ...
    def transferResource(self,resource_element,old_name,new_name):
        ...
        if fileext=='png':
            newfile = os.path.join(self._res_dir,STR_DIR_STUB,new_name,fname.replace(' ',''))
        else:
            newfile = os.path.join(self._res_dir,'music',fname.replace(' ',''))
...
class LevelWorld(ThingWorld):
    ...
    @property
    def isReadOnly( self ):
        return self.name.lower() in metawog.LEVELS_READONLY_LOWER
        ...
        if pipe is not None:
            ...
            if pipe_type not in metawog.PIPES_STANDARD.keys():
                ...
                for resource_id_stub in metawog.PIPE_RESOURCE_STUBS:
                    ...
                    if image_id not in self.list_world_identifiers('image'):
                        if image_id in self.list_identifiers('image'):
                            self.addGlobalError(403,image_id)
                        else:
                            self.addGlobalError(402,image_id)
    ...
    def hasscene_issue(self):
        ...
        for geomitem in root.findall('compositegeom'):
            ...
            geomstatic = geomitem.get_native('static',False)
            if not geomstatic:
             if abs(geomitem.get_native('rotation',0)) > 1:
            ...
            for geomchild in geomitem.getchildren():
                ...
                cgmass+=geomchild.get_native('mass',0.0)
            ...
            if not geomstatic and cgmass<1:
                 self.addSceneError(13,geomitem.get('id',''))
        ...
        for geomitem in geomitems:
            id = geomitem.get('id','')
            rs = geomitem.get_native('rotspeed',0)
            if rs!=0:
                rotspeedbodys.add(id)
                if abs(rs)<0.00049:
                    self.addSceneError(14,id)
            geomstatic = geomitem.get_native('static',False)
        ...
        #Scenelayer
        for scenelayer in root.findall('SceneLayer'):
            if scenelayer.get('anim','')!='':
                scalex,scaley = scenelayer.get_native('scale',(1,1))
                if abs((scalex/scaley)-1) > 0.05:
                   self.addSceneError(12,scenelayer.get('name',''))
    ...
    def _get_used_resources(self):
        ...
        for element in self.scene_root.findall('.//*'):
        ...
        for element in self.level_root.findall('.//*'):
          for attribute_meta in element.meta.attributes:
            if attribute_meta.type == metaworld.REFERENCE_TYPE:
                if attribute_meta.reference_family in oftype:
                    if element.get(attribute_meta.name):
...
#                              print "Is not Clone",path
    ...
    def _buildDependancyTree(self):
        ...
        dependancy_tree = self._universe.make_unattached_tree_from_xml( metawog.TREE_LEVEL_DEPENDANCY, metawog.LEVEL_DEPENDANCY_TEMPLATE )
        ...
        old_tree = self.find_tree(metawog.TREE_LEVEL_DEPENDANCY)
        if old_tree is None:
                        ...
                        for i in range(4):
                          if ball_element.tag=="ball":
                             ...
                             break
                              ...
                              break
                          #else:
                          #    print "ball_element =",ball_element.tag," parent=",ball_element.parent.tag,i
    ...
    def _cleanleveltree(self):
        self.suspend_undo()
        tree = self.find_tree(metawog.TREE_LEVEL_GAME)
        #new_tree = self._universe.make_unattached_tree_from_xml( metawog.TREE_LEVEL_GAME,
        #                    old_tree.to_xml() )
        ballids=set([''])
        for element in tree.root.getchildren():
            if element.tag in ['BallInstance']:
                ballids.add(element.get('id',''))
            elif element.tag=='Strand':
                ballid = element.get('gb1')
                if ballid not in ballids:
                    tree.root.remove(element)
                    tree.root.append(element)
                else:
                    ballid = element.get('gb2')
                    if ballid not in ballids:
                        tree.root.remove(element)
                        tree.root.append(element)
    ...
    def _cleanscenetree(self):
        self.suspend_undo()
        tree = self.find_tree(metawog.TREE_LEVEL_SCENE)
        #new_tree = self._universe.make_unattached_tree_from_xml( metawog.TREE_LEVEL_SCENE,
        #                    old_tree.to_xml() )
        geomids=set([''])
        for element in tree.root.findall('.//*'):
            if element.tag in ['rectangle','circle','compositegeom']:
                geomids.add(element.get('id',''))
            elif element.tag=='hinge':
                bodyid = element.get('body1','')
                if bodyid not in geomids:
                    tree.root.remove(element)
                    tree.root.append(element)
                else:
                    bodyid = element.get('body2','')
                    if bodyid not in geomids:
                        tree.root.remove(element)
                        tree.root.append(element)
            elif element.tag=='motor':
                bodyid = element.get('body','')
                if bodyid not in geomids:
                    tree.root.remove(element)
                    tree.root.append(element)
            elif element.tag=='button':
                bodyid = element.get('anchor','')
                if bodyid not in geomids:
                    if element.parent.tag=='buttongroup':
                        tree.root.remove(element.parent)
                        tree.root.append(element.parent)
                    else:
                        tree.root.remove(element)
                        tree.root.append(element)
    ...
    def updateResources( self ):
        ...
        """
        self._importError=[None,None]
        dir = os.path.normpath(os.path.join( self.game_model._res_dir , STR_DIR_STUB, self.name ))
        existing_paths = []
    ...
    def transferResource(self,filename):
        fileext = os.path.splitext(filename)[1][1:4].lower()
        res_path = filename[len(os.path.normpath(self.game_model._wog_dir))+1:]
        if self.game_model._isOriginalFile(os.path.splitext(res_path)[0],fileext):
            return filename

        #@DaB - Ensure if the file is copied that it's new extension is always lower case
        fname = os.path.splitext(os.path.split( filename )[1])[0] + "." + fileext
        if fileext=='png':
            if os.path.normpath(filename)[:len(os.path.normpath(self.game_model._res_dir))]==os.path.normpath(self.game_model._res_dir):
                print "Png File Already in Res folder"
                return filename
            newfile = os.path.join(self.game_model._res_dir,STR_DIR_STUB,self.name,fname.replace(' ',''))
        else:
            newfile = os.path.join(self.game_model._res_dir,'music',fname.replace(' ',''))
        if os.path.normpath(filename)!=os.path.normpath(newfile):
            basefile = os.path.splitext(newfile)
            serial = 2
            import_data = file(os.path.normpath(filename),'rb').read()
            importfilesize=len(import_data)
            while os.path.isfile(newfile):
                if os.path.getsize(newfile) == importfilesize:
                    if file(newfile,'rb').read() == import_data:
                        print "file exists as " + newfile
                        return newfile
                newfile = "%s%d%s" % (basefile[0],serial,basefile[1])
                serial+=1
    ...
    def addResources(self,filenames):
        ...
        added_elements = []
        resmap = {'png':('Image','IMAGE_SCENE_%s_%s','image'),'ogg':('Sound','SOUND_LEVEL_%s_%s','sound')}
    ...
    def checkResourceIds(self):
        print "Checking Resource Ids"
            ...
            if model is not None:
                ...
                if clonedfiles is not None:
                    txtIssue = self.tr("""<p>This level uses renamed / moved copies of original World of Goo files.<br>
                                        ...
                                        <p>%s</p>
                                        <p>If you want WooGLE to automatically fix this, click OK<br>Otherwise click Cancel to abort creating the goomod</p>"""
                                        % self.tr("<br>".join(clonedfiles.keys())) )
                    ret = QtGui.QMessageBox.warning(self, self.tr("This level uses cloned files!"),
                        txtIssue,QtGui.QMessageBox.Ok | QtGui.QMessageBox.Cancel )
                ...
                #MAC Compatible dir construction
                #dir = os.path.normpath(os.path.join( self._game_model._res_dir,STR_DIR_STUB, model.name))
                ...
                if filename!='':
                    ...
                    filename = os.path.normpath(unicode(filename))
...
#        dock.setObjectName('Library_tab')
#        dock.setAllowedAreas( Qt.RightDockWidgetArea )
#        self.libraryView = reslibrary.ResourceLibraryView()
#        dock.setWidget( self.libraryView )
#        self.addDockWidget( Qt.RightDockWidgetArea, dock )
#        self.tabifyDockWidget( dep_dock, dock )
#        dock.setFeatures(QtGui.QDockWidget.NoDockWidgetFeatures)
...
if __name__ == "__main__":
    ...
    #wogfile.restoreAlpha("e:\input.png", "e:\output.png")
    #wogfile.decomp("e:\chunk.dat", "e:\output.dat")
    #sys.exit(0)
