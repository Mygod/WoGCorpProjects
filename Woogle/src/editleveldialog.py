...
class Ui_EditLevelDialog(object):
    def setupUi(self, EditLevelDialog, all, original):
    ...
    def comboSelectionChanged(self,index):
        ...
        if index == 0:
            #All
          for level in sorted(self.all,key=unicode.lower):
            self.levelList.addItem( level )
        elif index == 1:
            #Custom
            custom_levels = sorted(self.all - self.original,key=unicode.lower)
            for level in custom_levels:
              self.levelList.addItem( level )
        elif index == 2:
            #Original
          for level in sorted(self.original,key=str.lower):
            self.levelList.addItem( level )
