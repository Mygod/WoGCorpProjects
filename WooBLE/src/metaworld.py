...
class containsAttributeMeta(AttributeMeta):
    def  is_valid_value( self, text, world ): #IGNORE:W0613
        status = AttributeMeta.is_valid_value(self, text, world)
        if status is None and text:
            message = 'Invalid contains : {Number},{Ball Name},{Number},{Ball Name}...'
            contains=text.split(',')
            if len(contains)%2!=0:
                return message,{}
            argcount=1
            for thing in contains:
                if (argcount%2)==1:
                    if not self._isNumber(thing.strip()):
                        return message,{}
                    if float(thing)<=0:
                        return message,{}
                else:
                    if thing.strip() not in BALL_NAMES:
                        return message,{}
                argcount+=1
        return status
...
class statescalesAttributeMeta(AttributeMeta):
    def  is_valid_value( self, text, world ): #IGNORE:W0613
        status = AttributeMeta.is_valid_value(self, text, world)
        if status is None and text:
            message = 'Invalid statescales : {StateName},{ScaleFactor},{StateName},{ScaleFactor}...'
            statescales=text.split(',')
            if len(statescales)%2!=0:
                return message,{}
            argcount=1
            for thing in statescales:
                if (argcount%2)==0:
                    if not self._isNumber(thing.strip()):
                        return message,{}
                    if float(thing)<=0:
                    ...
                    if thing.strip() not in BALL_STATES:
                        return message,{}
                argcount+=1
        return status
...
class shapeAttributeMeta(AttributeMeta):
    def  is_valid_value( self, text, world ): #IGNORE:W0613
        status = AttributeMeta.is_valid_value(self, text, world)
        if status is None and text:
            message = 'Invalid Shape : circle,{diameter} or rectangle,{width},{height}'
            shape=text.split(',')
    ...
    def validShape(self,shape):
        ...
        # expects tuple of values
        if len(shape) < 2:
            return
        if shape[0].strip() not in ['circle','rectangle']:
            return
        if shape[0].strip() == 'circle':
            if len(shape)>3:
                return
            if not self.validSize(shape[1]):
                return
            if len(shape)==3:
                if not self.validVariance(shape[2]):
                    return
        else:
            if len(shape)>4:
                return
            if not self.validSize(shape[1]):
                return
            if not self.validSize(shape[2]):
                return
            if len(shape)==4:
                if not self.validVariance(shape[3]):
                    return
        return True
    ...
    def validVariance(self,value):
        try:
            variance = float(value)
        except ValueError:
            return
        return (variance >= 0) and (variance <1)
...
class ReferenceAttributeMeta(ComponentsAttributeMeta):
    def __init__( self, name, reference_family, reference_world, is_list = False,
        ...
        if is_list and 'allow_empty' not in kwargs:
            kwargs['allow_empty'] = True
        if is_list and 'min_components' not in kwargs:
            min_components = 1
        if is_list and 'max_components' not in kwargs:
            max_components = None
        min_components = not is_list and 1 or min_components
        max_components = not is_list and 1 or max_components
...
class PathAttributeMeta(AttributeMeta):
    ...
    def  is_valid_value( self, text, world ): #IGNORE:W0613
        status = AttributeMeta.is_valid_value(self, text, world)
        if status is None and text:
            res_file = self._clean_path(text)
            if ' ' in res_file:
                message = 'Resource filenames cannot contains spaces : ' + res_file
                return message, {}

            filename = os.path.normpath(os.path.join(WOG_PATH,res_file+self.strip_extension))
...
def contains_attribute( name, **kwargs ):
    return containsAttributeMeta( name, STRING_TYPE, **kwargs )

def statescales_attribute( name, **kwargs ):
    return statescalesAttributeMeta( name, STRING_TYPE, **kwargs )

def shape_attribute( name, **kwargs ):
    return shapeAttributeMeta( name, STRING_TYPE, **kwargs )
